...
`loading` shows a leading spinner and sets `disabled` and `aria-busy` together.
`shape="pill"` forces a fully rounded button, and `render` is Base UI's escape
hatch for rendering as another element.

Picking a variant: `default`
