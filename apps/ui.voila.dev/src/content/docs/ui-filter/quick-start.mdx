...
- [Filter.Root](/ui-filter/filter-root): the whole surface: search, trigger, editor, chips, result count.
- [Filter.Form](/ui-filter/filter-form): the editor on its own, for a sidebar or a settings sheet.
- [Filter.Chips](/ui-filter/filter-chips): the active filters as removable chips.
- [Filter.Trigger](/ui-filter/filter-trigger): the search-shaped button that opens the editor.
