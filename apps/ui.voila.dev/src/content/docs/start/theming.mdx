...
description: "Override the CSS custom properties in @voila.dev/ui-tokens to rebrand the whole system without forking a single component."
---

import Preview from "../../../components/preview.astro";
import * as Example from "../../../examples/theming";

The palette that ships with `@voila.dev/ui-tokens` is a **default, not a
brand**. Every colour, radius and font is a CSS custom property, and every
component reads them through Tailwind's semantic class names (`bg-primary`,
`text-muted-foreground`, `border-border`). Redefine the properties and the whole
system moves with them — no component is forked, no class is overridden.
...
Redefine the properties *after* importing the stylesheet. Set both themes.

```css title="src/styles/globals.css"
@import "tailwindcss";
@import "@voila.dev/ui/styles/globals.css";
@source "./node_modules/@voila.dev/ui/src";
...
`--provider` (blue) and `--organization` (orange) exist for apps with two
...
<Preview label="Badges are data-coloured, not semantic">
	<Example.BadgeColors client:load />
</Preview>
...
<Preview label="What moves when --primary and --radius move">
	<Example.TokenSurface client:load />
</Preview>
