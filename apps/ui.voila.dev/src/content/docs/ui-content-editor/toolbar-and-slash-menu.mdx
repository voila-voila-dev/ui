...
`ContentEditor.Toolbar` with no children shows every item the registry's
features declare, grouped in a fixed order: history, text, block, list,
insert, table. An item is a `Toggle` when it knows whether it is active, a
ghost `Button` otherwise, with a `Tooltip` carrying its label and its keys,
and a `Popover` when it opens a form. An item that requires a capability the
host did not wire (image upload) is absent, not disabled.
