import type { Meta, StoryObj } from "@storybook/tanstack-react";
import {
	type EmailEditorDocument,
	emptyEmailEditorDocument,
} from "@voila.dev/ui-email-block-editor/document/types";
import { EmailBlockEditor } from "@voila.dev/ui-email-block-editor/email-block-editor";
import { useState } from "react";

const meta = {
	title: "EmailBlockEditor/EmailBlockEditor",
	component: EmailBlockEditor,
	tags: ["autodocs"],
	parameters: { layout: "fullscreen" },
} satisfies Meta<typeof EmailBlockEditor>;

export default meta;

type Story = StoryObj<typeof meta>;

const sampleDocument: EmailEditorDocument = {
	version: 1,
	blocks: [
		{
			id: "heading",
			type: "heading",
			text: "Bonjour {{firstName}} !",
			level: 1,
		},
		{
			id: "intro",
			type: "paragraph",
			spans: [
				{ text: "Découvrez les " },
				{ text: "nouvelles missions", bold: true },
				{ text: " disponibles près de chez vous cette semaine sur " },
				{ text: "votre espace", href: "https://app.acme.dev" },
				{ text: "." },
			],
		},
		{
			id: "cta",
			type: "button",
			label: "Voir les missions",
			href: "https://app.acme.dev/missions",
			align: "center",
			variant: "primary",
		},
		{ id: "divider", type: "divider" },
		{
			id: "outro",
			type: "paragraph",
			spans: [{ text: "À très vite sur Acme." }],
		},
	],
};

/** Fake upload for the stories: no backend here, so the picked file is served
 * from an object URL — the real admin flow returns the uploaded file's URL. */
const fakeUploadImage = async (file: File) => {
	await new Promise((resolve) => setTimeout(resolve, 600));
	return URL.createObjectURL(file);
};

function ControlledEditor({ document }: { document: EmailEditorDocument }) {
	const [value, setValue] = useState(document);
	return (
		<div className="p-6">
			<EmailBlockEditor
				document={value}
				onChange={setValue}
				onUploadImage={fakeUploadImage}
			/>
		</div>
	);
}

export const Composed: Story = {
	args: {
		document: sampleDocument,
		onChange: () => {},
	},
	render: (args) => <ControlledEditor document={args.document} />,
};

/** A multi-column row: blocks drag in and out of the cells, and the column
 * counts (desktop and mobile) live in « Réglages du bloc ». */
const gridDocument: EmailEditorDocument = {
	version: 1,
	blocks: [
		{ id: "heading", type: "heading", text: "Nos offres", level: 1 },
		{
			id: "row",
			type: "grid",
			desktopColumns: 3,
			mobileColumns: 1,
			children: [
				{
					id: "cell-1",
					type: "image",
					src: "https://placehold.co/162x120/png",
					alt: "Offre découverte",
					href: "https://app.acme.dev",
					width: "full",
					overlay: "none",
					rounded: true,
				},
				{ id: "cell-2", type: "heading", text: "Essentiel", level: 2 },
				{
					id: "cell-3",
					type: "paragraph",
					spans: [{ text: "Le socle pour démarrer." }],
				},
			],
		},
		{
			id: "cta",
			type: "button",
			label: "Découvrir",
			href: "https://app.acme.dev",
			align: "center",
			variant: "secondary",
		},
	],
};

export const Grid: Story = {
	args: { document: gridDocument, onChange: () => {} },
	render: (args) => <ControlledEditor document={args.document} />,
};

export const Empty: Story = {
	args: {
		document: emptyEmailEditorDocument(),
		onChange: () => {},
	},
	render: (args) => <ControlledEditor document={args.document} />,
};

/** One of every block, for the mobile checklist: no horizontal overflow, the
 * toolbar inside the viewport, 44px tap targets, reachable settings. */
const everyBlockDocument: EmailEditorDocument = {
	version: 1,
	blocks: [
		{ id: "h1", type: "heading", text: "Bonjour {{firstName}} !", level: 1 },
		{
			id: "intro",
			type: "paragraph",
			spans: [
				{ text: "Le récapitulatif de " },
				{ text: "votre mois", bold: true },
				{ text: " sur Acme." },
			],
		},
		{
			id: "stats",
			type: "grid",
			desktopColumns: 3,
			mobileColumns: 2,
			children: [
				{
					id: "s1",
					type: "stat",
					value: "1 240",
					label: "Missions",
					description: "",
					align: "center",
				},
				{
					id: "s2",
					type: "stat",
					value: "98 %",
					label: "Satisfaction",
					description: "",
					align: "center",
				},
				{
					id: "s3",
					type: "stat",
					value: "4,8",
					label: "Note moyenne",
					description: "",
					align: "center",
				},
			],
		},
		{ id: "d1", type: "divider" },
		{ id: "h2", type: "heading", text: "Comment ça marche", level: 2 },
		{
			id: "steps",
			type: "list",
			marker: "badge",
			items: [
				{
					title: "Publiez votre mission",
					spans: [{ text: "En deux minutes." }],
				},
				{
					title: "Recevez des candidatures",
					spans: [{ text: "De professionnels vérifiés." }],
				},
			],
		},
		{
			id: "img",
			type: "image",
			src: "https://placehold.co/536x220/png",
			alt: "Une séance",
			href: "https://acme.dev",
			width: "full",
			overlay: "play",
			rounded: true,
		},
		{
			id: "cards",
			type: "grid",
			desktopColumns: 2,
			mobileColumns: 1,
			children: [
				{
					id: "art",
					type: "article",
					title: "Bien choisir son kiné",
					description: "Les trois critères qui comptent.",
					image: { src: "https://placehold.co/252x120/png", alt: "" },
					author: "Emma Martin",
					publishDate: "2026-07-20",
					href: "https://acme.dev/blog",
				},
				{
					id: "prod",
					type: "product",
					name: "Bande de contention",
					description: "Boîte de 10.",
					image: { src: "https://placehold.co/252x120/png", alt: "" },
					price: { amountInMinorUnits: 5990, currency: "EUR" },
					compareAtPrice: { amountInMinorUnits: 7990, currency: "EUR" },
					href: "https://shop.acme.dev",
					buttonLabel: "Commander",
				},
			],
		},
		{
			id: "tarifs",
			type: "table",
			headerRow: true,
			columns: [
				{ label: "Prestation", align: "left" },
				{ label: "Tarif", align: "right" },
			],
			rows: [
				["Kinésithérapie", "60,00 €"],
				["Ostéopathie", "55,00 €"],
			],
		},
		{
			id: "offre",
			type: "offer",
			eyebrow: "Le plus choisi",
			name: "Essentiel",
			description: "",
			image: { src: "", alt: "" },
			price: { amountInMinorUnits: 2900, currency: "EUR" },
			period: "par mois",
			features: ["Missions illimitées", "Support 7j/7"],
			buttonLabel: "Choisir",
			buttonHref: "https://acme.dev/offres",
			highlighted: true,
		},
		{
			id: "note",
			type: "rating",
			question: [{ text: "Que pensez-vous de Acme ?" }],
			style: "filled",
			lowLabel: "Pas du tout",
			highLabel: "Tout à fait",
			href: "https://acme.dev/avis",
		},
		{
			id: "cta",
			type: "button",
			label: "Ouvrir mon espace",
			href: "https://app.acme.dev",
			align: "center",
			variant: "primary",
		},
	],
};

export const EveryBlock: Story = {
	args: { document: everyBlockDocument, onChange: () => {} },
	render: (args) => <ControlledEditor document={args.document} />,
};
