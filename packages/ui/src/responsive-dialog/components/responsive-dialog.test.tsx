...
function Fixture({
	...
	onOpenChange,
	...props
}: React.ComponentProps<typeof ResponsiveDialog.Content> & {
	footerCloseLabel?: string;
	onOpenChange?: (open: boolean) => void;
}) {
	return (
		<ResponsiveDialog.Root onOpenChange={onOpenChange}>
			<ResponsiveDialog.Trigger render={<Button variant="outline" />}>
				Cancel project
			</ResponsiveDialog.Trigger>
			<ResponsiveDialog.Content {...props}>
				...
				<ResponsiveDialog.Footer closeLabel={footerCloseLabel}>
...
describe("ResponsiveDialog", () => {
	describe("on desktop", () => {
		...
		it("closes through ResponsiveDialog.Close", async () => {
			const screen = render(<Fixture />);
			fireEvent.click(screen.getByRole("button", { name: "Cancel project" }));
			await waitFor(() => {
	...
	it("throws when a part is used outside the root", () => {
		const consoleError = vi
			...
			.mockImplementation(() => {});
		expect(() =>
			render(<ResponsiveDialog.Trigger>Open</ResponsiveDialog.Trigger>),
		).toThrowError(/within <ResponsiveDialog.Root>/);
