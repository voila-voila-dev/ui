...
import { Rating, RatingInput, ReviewItem } from "#/components/ui/rating.tsx";
...
describe("Rating", () => {
	it("renders five stars by default", () => {
		const screen = render(<Rating value={3} />);
	...
	it("rounds fractional values for the fill count", () => {
		const screen = render(<Rating value={3.6} />);
		const filled = screen.baseElement.querySelectorAll(
			"[data-slot=rating-star][data-filled]",
		);
	...
	it("clamps values above the maximum", () => {
		const screen = render(<Rating value={9} />);
		const filled = screen.baseElement.querySelectorAll(
			"[data-slot=rating-star][data-filled]",
		);
	...
	it("exposes an accessible label", () => {
		const screen = render(<Rating value={4} />);
	...
	it("renders the review count when provided", () => {
		const screen = render(<Rating value={4} count={128} />);
...
describe("RatingInput", () => {
	it("renders a radiogroup with one radio per star", () => {
		const screen = render(<RatingInput value={0} onChange={() => {}} />);
	...
	it("marks the selected star as checked", () => {
		const screen = render(<RatingInput value={3} onChange={() => {}} />);
	...
	it("calls onChange with the clicked star value", () => {
		...
		const screen = render(<RatingInput value={0} onChange={onChange} />);
		const stars = allBySlot(screen, "rating-input-star");
	...
	it("emits a hidden input when name is set", () => {
		const screen = render(
			<RatingInput value={2} onChange={() => {}} name="score" />,
		);
