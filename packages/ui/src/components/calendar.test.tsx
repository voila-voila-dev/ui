...
describe("Calendar", () => {
	it("renders a calendar root slot", () => {
		const screen = render(<Calendar mode="single" defaultMonth={JUNE_2026} />);
	...
	it("marks the selected day with data-selected-single", () => {
		...
		const screen = render(
			<Calendar mode="single" selected={selected} defaultMonth={JUNE_2026} />,
		);
	...
	it("renders week numbers when showWeekNumber is set", () => {
		const screen = render(
			<Calendar mode="single" showWeekNumber defaultMonth={JUNE_2026} />,
		);
