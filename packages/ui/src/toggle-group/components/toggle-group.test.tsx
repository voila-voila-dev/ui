...
import { ToggleGroup } from "#/toggle-group/components/toggle-group.tsx";

afterEach(cleanup);

function Fixture(props: React.ComponentProps<typeof ToggleGroup.Root>) {
	return (
		<ToggleGroup.Root defaultValue={["week"]} {...props}>
...
describe("ToggleGroup", () => {
	...
	it("lets an item style itself when the group sets no variant", () => {
		const screen = render(
			<ToggleGroup.Root>
				<ToggleGroup.Item value="day" variant="outline" size="lg">
					Day
				</ToggleGroup.Item>
			</ToggleGroup.Root>,
		);
	...
	it("does not toggle a disabled item", () => {
		...
		const screen = render(
			<ToggleGroup.Root onValueChange={onValueChange}>
				<ToggleGroup.Item value="day" disabled>
					Day
				</ToggleGroup.Item>
			</ToggleGroup.Root>,
		);
	...
	it("merges className on the group and items", () => {
		const screen = render(
			<ToggleGroup.Root className="custom-group">
				<ToggleGroup.Item value="day" className="custom-item">
					Day
				</ToggleGroup.Item>
			</ToggleGroup.Root>,
		);
