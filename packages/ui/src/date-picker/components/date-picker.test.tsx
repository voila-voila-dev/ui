...
describe("DatePicker", () => {
	it("shows the placeholder and marks the trigger empty when nothing is selected", () => {
		const screen = render(<DatePicker.Root placeholder="Project date" />);
	...
	it("formats the selected date on the trigger (Intl long date, en default)", () => {
		const screen = render(<DatePicker.Root defaultValue={JUNE_20} />);
		const trigger = queryTrigger(screen);
	...
	it("formats the trigger label with the provided locale", () => {
		const screen = render(
			<DatePicker.Root locale="fr-FR" defaultValue={JUNE_20} />,
		);
	...
	it("opens the calendar popover on trigger click", async () => {
		const screen = render(
			<DatePicker.Root calendarProps={{ defaultMonth: JUNE_2026 }} />,
		);
	...
	it("serializes the value into a hidden form input as yyyy-MM-dd", () => {
		const screen = render(
			<DatePicker.Root name="projectDate" defaultValue={JUNE_20} />,
		);
	...
	it("does not open when disabled", async () => {
		const screen = render(<DatePicker.Root disabled />);
	...
	it("merges className and forwards aria-invalid onto the trigger", () => {
		const screen = render(
			<DatePicker.Root className="custom-picker" aria-invalid />,
		);
		const trigger = queryTrigger(screen);
