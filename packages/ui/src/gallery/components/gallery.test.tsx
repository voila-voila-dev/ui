...
import { Gallery } from "#/gallery/components/gallery.tsx";
...
describe("Gallery", () => {
	it("renders a thumbnail per image", () => {
		const screen = render(<Gallery.Root images={IMAGES} />);
	...
	it("renders an Empty state when there are no images", () => {
		const screen = render(
			<Gallery.Root images={[]} emptyLabel="Aucune photo" />,
		);
	...
	it("merges a consumer className into the root", () => {
		const screen = render(
			<Gallery.Root images={IMAGES} className="custom-gallery" />,
		);
		expect(
...
describe("Lightbox", () => {
	it("does not render content while closed", () => {
		const screen = render(
			<Gallery.Lightbox images={IMAGES} open={false} onOpenChange={() => {}} />,
		);
		expect(queryBySlot(screen, "dialog-content")).toBeNull();
	});
	...
	it("renders the images when open", () => {
		const screen = render(
			<Gallery.Lightbox images={IMAGES} open onOpenChange={() => {}} />,
		);
	...
	it("calls onOpenChange when dismissed", () => {
		...
		const screen = render(
			<Gallery.Lightbox images={IMAGES} open onOpenChange={onOpenChange} />,
		);
