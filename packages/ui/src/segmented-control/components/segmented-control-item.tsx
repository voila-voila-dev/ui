...
export function SegmentedControlItem({ className, ...props }: Props) {
	const { size, stretch } = React.useContext(SegmentedControlContext);
	...
	return (
		<RadioPrimitive.Root
			...
			className={cn(
				...
				// An equal share of a stretched root, rather than each segment
				// keeping the width of its own label.
				stretch && "flex-1",
				className,
			)}
			{...props}
