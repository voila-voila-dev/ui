...
describe("Progress", () => {
	it("exposes progressbar semantics for a determinate value", () => {
		const screen = render(<Progress.Root value={35} />);
	...
	it("marks the parts indeterminate when value is null", () => {
		const screen = render(<Progress.Root value={null} />);
	...
	it("marks the root complete at the maximum value", () => {
		const screen = render(<Progress.Root value={100} />);
		const root = screen.baseElement.querySelector("[data-slot=progress]");
		expect(root?.hasAttribute("data-complete")).toBe(true);
	});

	it("labels the bar through Progress.Label", () => {
		const screen = render(
	...
	it("renders the formatted value through Progress.Value", () => {
		const screen = render(
			<Progress.Root value={35}>
				<Progress.Value data-testid="readout" />
			</Progress.Root>,
		);
	...
	it("renders a custom value through the Progress.Value render function", () => {
		const screen = render(
