...
function Cropper(props: React.ComponentProps<typeof ImageCropper.Root>) {
	return (
		<ImageCropper.Root {...props}>
...
describe("ImageCropper composition", () => {
	...
	it("removes the image, revokes its object URL and reports null", () => {
		const onImageChange = vi.fn();
		const screen = render(
			<ImageCropper.Root onImageChange={onImageChange}>
				<ImageCropper.Dropzone />
				<ImageCropper.Area />
				<RemoveButton />
			</ImageCropper.Root>,
	...
	it("throws when cropper parts are used outside <ImageCropper.Root>", () => {
		const consoleError = vi
			.spyOn(console, "error")
			.mockImplementation(() => {});
		expect(() => render(<ImageCropper.Area />)).toThrow(
			"ImageCropper.Area must be used within <ImageCropper.Root>",
		);
