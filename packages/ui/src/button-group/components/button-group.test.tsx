...
import { ButtonGroup } from "#/button-group/components/button-group.tsx";
...
describe("ButtonGroup", () => {
	...
	it("exposes the vertical orientation as a data attribute", () => {
		const screen = render(
			<ButtonGroup.Root aria-label="Actions" orientation="vertical">
				<button type="button">One</button>
			</ButtonGroup.Root>,
		);
		const group = screen.getByRole("group", { name: "Actions" });
	...
	it("merges className onto the group element", () => {
		const screen = render(
			<ButtonGroup.Root aria-label="Actions" className="custom-group-class" />,
		);
		const group = screen.getByRole("group", { name: "Actions" });
...
describe("ButtonGroup.Text", () => {
	it("renders a div with its slot attribute and children", () => {
		const screen = render(<ButtonGroup.Text>June 2026</ButtonGroup.Text>);
	...
	it("renders as a label through the render prop", () => {
		const screen = render(
			...
			<ButtonGroup.Text render={<label />}>Period</ButtonGroup.Text>,
		);
		const text = screen.baseElement.querySelector(
			"[data-slot=button-group-text]",
		);
