...
import { RadioGroup } from "#/radio-group/components/radio-group.tsx";
...
function renderGroup(props?: React.ComponentProps<typeof RadioGroup.Root>) {
	return render(
...
describe("RadioGroup", () => {
	...
	it("forwards aria-invalid to the item", () => {
		const screen = render(
			<RadioGroup.Root>
				<RadioGroup.Item value="a" aria-invalid />
			</RadioGroup.Root>,
		);
	...
	it("merges className over the item classes", () => {
		const screen = render(
			<RadioGroup.Root>
				<RadioGroup.Item value="a" className="custom-item-class" />
			</RadioGroup.Root>,
		);
		const [item] = queryItems(screen);
...
function renderCards(props?: React.ComponentProps<typeof RadioGroup.Root>) {
	return render(
		<RadioGroup.Root {...props}>
			<RadioGroup.Card value="starter">Starter</RadioGroup.Card>
			<RadioGroup.Card value="pro">Pro</RadioGroup.Card>
		</RadioGroup.Root>,
	);
}

describe("RadioGroup.Card", () => {
	...
	it("hides the corner indicator and its reserved padding via showIndicator", () => {
		const screen = render(
			<RadioGroup.Root defaultValue="starter">
				<RadioGroup.Card value="starter" showIndicator={false}>
					Starter
				</RadioGroup.Card>
			</RadioGroup.Root>,
		);
	...
	it("blocks selection while disabled", () => {
		const screen = render(
			<RadioGroup.Root defaultValue="starter">
				<RadioGroup.Card value="starter">Starter</RadioGroup.Card>
				<RadioGroup.Card value="pro" disabled>
					Pro
				</RadioGroup.Card>
			</RadioGroup.Root>,
		);
	...
	it("merges className over the card classes", () => {
		const screen = render(
			<RadioGroup.Root>
				<RadioGroup.Card value="starter" className="custom-card-class">
					Starter
				</RadioGroup.Card>
			</RadioGroup.Root>,
		);
