...
import { ScrollArea } from "#/scroll-area/components/scroll-area.tsx";
...
describe("ScrollArea", () => {
	...
	it("forwards a horizontal orientation to a composed ScrollArea.Bar", () => {
		const screen = render(
			<ScrollArea.Root>
				<p>Content</p>
				<ScrollArea.Bar orientation="horizontal" keepMounted />
			</ScrollArea.Root>,
		);
	...
	it("merges className on the root", () => {
		const screen = render(
			<ScrollArea.Root className="h-56 w-72 custom-scroll-area">
				<p>Content</p>
			</ScrollArea.Root>,
		);
		const root = screen.baseElement.querySelector("[data-slot=scroll-area]");
	...
	it("merges className on the scrollbar", () => {
		const screen = render(
			<ScrollArea.Root>
				<p>Content</p>
				<ScrollArea.Bar className="custom-scrollbar" keepMounted />
			</ScrollArea.Root>,
		);
