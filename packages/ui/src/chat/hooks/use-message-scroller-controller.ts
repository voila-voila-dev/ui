...
import { useMessageScrollerCommands } from "#/chat/hooks/use-message-scroller-commands.ts";
import { useMessageScrollerRefs } from "#/chat/hooks/use-message-scroller-refs.ts";
import {
	getContentBottom,
	getElementTop,
...
} from "#/chat/lib/message-scroller-geometry.ts";
import type {
...
function useMessageScrollerController({
	...
	const handleResize = React.useCallback(() => {
		...
		const previousSpacerHeight = refs.spacerHeightRef.current;
		...
		if (reanchorToAnchoredMessage()) {
			...
			// from the anchor hold to following the bottom. Requiring the >0 to 0
			...
			if (
				...
				refs.spacerHeightRef.current === 0
			) {
				scrollToEnd({ behavior: "auto" });
			}
	...
	const observeVisibility = React.useCallback(() => {
		...
		for (const element of messageElementsRef.current.values()) {
