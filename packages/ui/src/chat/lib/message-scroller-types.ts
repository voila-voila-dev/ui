...
/**
 * Headless message-scroller primitive, vendored from shadcn/ui
 * (`@shadcn/react/message-scroller`) so the chat kit carries no external
 * runtime dependency. Adapted to Base UI's `useRender`/`mergeProps` (already a
 * dependency of this package) and the repo's formatting; the behavior is
 * upstream's: follow-bottom, turn anchoring, prepend preservation, visibility
 * tracking.
 */
...
// Scroll snapshot from useChatScrollerScrollable: which edges the viewport can
// still scroll toward.
...
// Visibility snapshot from useChatScrollerVisibility.
...
// Minimal external store backing useChatScrollerScrollable, consumed via
...
// External store backing useChatScrollerVisibility. Adds reference-counting so
// tracking stays lazy.
